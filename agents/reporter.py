...
def build_markdown_report(result: AuditResult) -> str:
    ...
    if deps:
        ...
        for d in deps:
            ...
            lines.append(
                f"| `{d.package_name}` | {d.ecosystem} | {exists_icon} | {cves} | {reason} |"
            )
        lines.append("")
    ...
    if vulns:
        ...
        for i, v in enumerate(
            sorted(vulns, key=lambda x: list(Severity).index(x.severity)), 1
        ):
            ...
            lines.append(
                f"**파일:** `{v.file_path}`"
                + (f" (라인 {v.line_number})" if v.line_number else "")
            )

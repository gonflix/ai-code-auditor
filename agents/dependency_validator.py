...
TYPOSQUAT_MAP = {
    typo: correct
    for correct, typos in KNOWN_TYPOSQUATS.items()
    for typo in typos
}
...
def validate_dependencies(files: list[ChangedFile]) -> list[DependencyRisk]:
    ...
    for file in files:
        ...
        for pkg in packages:
            ...
            if not exists:
                risk_reason = f"패키지 '{pkg}'가 {ecosystem}에 존재하지 않음 — 슬롭스쿼팅 위험"
            elif is_typo:
                correct = TYPOSQUAT_MAP[pkg]
                risk_reason = f"'{pkg}'는 '{correct}'의 오타일 가능성 — 악성 패키지 설치 위험"
...
_STDLIB = {
    ...
    "itertools", "functools", "pathlib", "io", "abc", "copy", "enum",
    "typing", "dataclasses", "contextlib", "logging", "unittest", "hashlib",
    "hmac", "base64", "struct", "socket", "threading", "subprocess",
    "multiprocessing", "asyncio", "http", "urllib", "email", "html",
    "xml", "csv", "sqlite3", "random", "string", "textwrap", "traceback",
    "inspect", "importlib", "pkgutil", "warnings", "weakref", "gc",
}
